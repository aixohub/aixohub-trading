...
from futu import OpenSecTradeContext, SecurityFirm, OpenFutureTradeContext, OpenQuoteContext
...
class FutuOrder(OrderBase, ib.ext.Order.Order):
...
class FutuBroker(with_metaclass(MetaFutuBroker, BrokerBase)):
    ...
    def __init__(self, **kwargs):
        ...
        self.futu_ctx = futustore.FutuStore(**kwargs)
    ...
    def start(self):
        ...
        self.futu_ctx.start(broker=self)

        if self.futu_ctx.connected():
            self.futu_ctx.reqAccountUpdates()
            self.startingcash = self.cash = self.futu_ctx.get_acc_cash()
            self.startingvalue = self.value = self.futu_ctx.get_acc_value()
        else:
            self.startingcash = self.cash = 0.0
            self.startingvalue = self.value = 0.0
    ...
    def getcash(self):
        ...
        self.cash = self.futu_ctx.get_acc_cash()
        return self.cash

    def getvalue(self, datas=None):
        self.value = self.futu_ctx.get_acc_value()
        return self.value

    def getposition(self, data, clone=True):
        return self.futu_ctx.getposition(data.tradecontract, clone=clone)
    ...
    def cancel(self, order):
        ...
        self.futu_ctx.cancelOrder(order.m_orderId)
    ...
    def submit(self, order):
        ...
        self.futu_ctx.placeOrder(order.m_orderId, order.data.tradecontract, order)
        self.notify(order)
    ...
    def _makeorder(self, action, owner, data,
        ...
        order = FutuOrder(action, owner=owner, data=data,
                          ...
                          m_orderId=self.futu_ctx.nextOrderId(),
                          **kwargs)
