...
class Alphas101():
    ...
    # Alpha#5	 (rank((open - (sum(vwap.py, 10) / 10))) * (-1 * abs(rank((close - vwap.py)))))
    def alpha005(self):
    ...
    # Alpha#11	 ((rank(ts_max((vwap.py - close), 3)) + rank(ts_min((vwap.py - close), 3))) *rank(delta(volume, 3)))
    def alpha011(self):
    ...
    # Alpha#25	 rank(((((-1 * returns) * adv20) * vwap.py) * (high - close)))
    def alpha025(self):
        adv20 = sma(self.volume, 20)
    ...
    # Alpha#27	 ((0.5 < rank((sum(correlation(rank(volume), rank(vwap.py), 6), 2) / 2.0))) ? (-1 * 1) : 1)
    def alpha027(self):
    ...
    # Alpha#32	 (scale(((sum(close, 7) / 7) - close)) + (20 * scale(correlation(vwap.py, delay(close, 5),230))))
    def alpha032(self):
    ...
    # Alpha#36	 (((((2.21 * rank(correlation((close - open), delay(volume, 1), 15))) + (0.7 * rank((open- close)))) + (0.73 * rank(Ts_Rank(delay((-1 * returns), 6), 5)))) + rank(abs(correlation(vwap.py,adv20, 6)))) + (0.6 * rank((((sum(close, 200) / 200) - open) * (close - open)))))
    def alpha036(self):
        adv20 = sma(self.volume, 20)
    ...
    # Alpha#41	 (((high * low)^0.5) - vwap.py)
    def alpha041(self):
        return pow((self.high * self.low), 0.5) - self.vwap

    # Alpha#42	 (rank((vwap.py - close)) / rank((vwap.py + close)))
    def alpha042(self):
    ...
    # Alpha#47	 ((((rank((1 / close)) * volume) / adv20) * ((high * rank((high - close))) / (sum(high, 5) /5))) - rank((vwap.py - delay(vwap.py, 5))))
    def alpha047(self):
        adv20 = sma(self.volume, 20)
    ...
    # Alpha#50	 (-1 * ts_max(rank(correlation(rank(volume), rank(vwap.py), 5)), 5))
    def alpha050(self):
    ...
    # Alpha#57	 (0 - (1 * ((close - vwap.py) / decay_linear(rank(ts_argmax(close, 30)), 2))))
    def alpha057(self):
        return (0 - (1 * ((self.close - self.vwap) / decay_linear(rank(ts_argmax(self.close, 30)), 2))))

    # Alpha#58	 (-1 * Ts_Rank(decay_linear(correlation(IndNeutralize(vwap.py, IndClass.sector), volume,3.92795), 7.89291), 5.50322))

    # Alpha#59	 (-1 * Ts_Rank(decay_linear(correlation(IndNeutralize(((vwap.py * 0.728317) + (vwap.py *(1 - 0.728317))), IndClass.industry), volume, 4.25197), 16.2289), 8.19648))
    ...
    # Alpha#61	 (rank((vwap.py - ts_min(vwap.py, 16.1219))) < rank(correlation(vwap.py, adv180, 17.9282)))
    def alpha061(self):
        adv180 = sma(self.volume, 180)
        return (rank((self.vwap - ts_min(self.vwap, 16))) < rank(correlation(self.vwap, adv180, 18))).astype('int')

    # Alpha#62	 ((rank(correlation(vwap.py, sum(adv20, 22.4101), 9.91009)) < rank(((rank(open) +rank(open)) < (rank(((high + low) / 2)) + rank(high))))) * -1)
    def alpha062(self):
        adv20 = sma(self.volume, 20)
    ...
    # Alpha#63	 ((rank(decay_linear(delta(IndNeutralize(close, IndClass.industry), 2.25164), 8.22237))- rank(decay_linear(correlation(((vwap.py * 0.318108) + (open * (1 - 0.318108))), sum(adv180,37.2467), 13.557), 12.2883))) * -1)

    # Alpha#64	 ((rank(correlation(sum(((open * 0.178404) + (low * (1 - 0.178404))), 12.7054),sum(adv120, 12.7054), 16.6208)) < rank(delta(((((high + low) / 2) * 0.178404) + (vwap.py * (1 -0.178404))), 3.69741))) * -1)
    def alpha064(self):
        ...
        # Alpha#65	 ((rank(correlation(((open * 0.00817205) + (vwap.py * (1 - 0.00817205))), sum(adv60,8.6911), 6.40374)) < rank((open - ts_min(open, 13.635)))) * -1)
    ...
    # Alpha#66	 ((rank(decay_linear(delta(vwap.py, 3.51013), 7.23052)) + Ts_Rank(decay_linear(((((low* 0.96633) + (low * (1 - 0.96633))) - vwap.py) / (open - ((high + low) / 2))), 11.4157), 6.72611)) * -1)
    def alpha066(self):
    ...
    # Alpha#67	 ((rank((high - ts_min(high, 2.14593)))^rank(correlation(IndNeutralize(vwap.py,IndClass.sector), IndNeutralize(adv20, IndClass.subindustry), 6.02936))) * -1)
    ...
    # Alpha#69	 ((rank(ts_max(delta(IndNeutralize(vwap.py, IndClass.industry), 2.72412),4.79344))^Ts_Rank(correlation(((close * 0.490655) + (vwap.py * (1 - 0.490655))), adv20, 4.92416),9.0615)) * -1)

    # Alpha#70	 ((rank(delta(vwap.py, 1.29456))^Ts_Rank(correlation(IndNeutralize(close,IndClass.industry), adv50, 17.8256), 17.9171)) * -1)

    # Alpha#71	 max(Ts_Rank(decay_linear(correlation(Ts_Rank(close, 3.43976), Ts_Rank(adv180,12.0647), 18.0175), 4.20501), 15.6948), Ts_Rank(decay_linear((rank(((low + open) - (vwap.py +vwap.py)))^2), 16.4662), 4.4388))
    def alpha071(self):
        adv180 = sma(self.volume, 180)
        ...
        return max(p1, p2)
        # return max(ts_rank(decay_linear(correlation(ts_rank(self.close, 3), ts_rank(adv180,12), 18).to_frame(), 4).CLOSE, 16), ts_rank(decay_linear((rank(((self.low + self.open) - (self.vwap.py +self.vwap.py))).pow(2)).to_frame(), 16).CLOSE, 4))

    # Alpha#72	 (rank(decay_linear(correlation(((high + low) / 2), adv40, 8.93345), 10.1519)) /rank(decay_linear(correlation(Ts_Rank(vwap.py, 3.72469), Ts_Rank(volume, 18.5188), 6.86671),2.95011)))
    def alpha072(self):
    ...
    # Alpha#73	 (max(rank(decay_linear(delta(vwap.py, 4.72775), 2.91864)),Ts_Rank(decay_linear(((delta(((open * 0.147155) + (low * (1 - 0.147155))), 2.03608) / ((open *0.147155) + (low * (1 - 0.147155)))) * -1), 3.33829), 16.7411)) * -1)
    def alpha073(self):
        ...
        return -1 * max(p1, p2)
        # return (max(rank(decay_linear(delta(self.vwap.py, 5).to_frame(), 3).CLOSE),ts_rank(decay_linear(((delta(((self.open * 0.147155) + (self.low * (1 - 0.147155))), 2) / ((self.open *0.147155) + (self.low * (1 - 0.147155)))) * -1).to_frame(), 3).CLOSE, 17)) * -1)

    # Alpha#74	 ((rank(correlation(close, sum(adv30, 37.4843), 15.1365)) <rank(correlation(rank(((high * 0.0261661) + (vwap.py * (1 - 0.0261661)))), rank(volume), 11.4791)))* -1)
    def alpha074(self):
    ...
    # Alpha#75	 (rank(correlation(vwap.py, volume, 4.24304)) < rank(correlation(rank(low), rank(adv50),12.4413)))
    def alpha075(self):
    ...
    # Alpha#76	 (max(rank(decay_linear(delta(vwap.py, 1.24383), 11.8259)),Ts_Rank(decay_linear(Ts_Rank(correlation(IndNeutralize(low, IndClass.sector), adv81,8.14941), 19.569), 17.1543), 19.383)) * -1)

    # Alpha#77	 min(rank(decay_linear(((((high + low) / 2) + high) - (vwap.py + high)), 20.0451)),rank(decay_linear(correlation(((high + low) / 2), adv40, 3.1614), 5.64125)))
    def alpha077(self):
        adv40 = sma(self.volume, 40)
        ...
        return min(p1, p2)
        # return min(rank(decay_linear(((((self.high + self.low) / 2) + self.high) - (self.vwap.py + self.high)).to_frame(), 20).CLOSE),rank(decay_linear(correlation(((self.high + self.low) / 2), adv40, 3).to_frame(), 6).CLOSE))

    # Alpha#78	 (rank(correlation(sum(((low * 0.352233) + (vwap.py * (1 - 0.352233))), 19.7428),sum(adv40, 19.7428), 6.83313))^rank(correlation(rank(vwap.py), rank(volume), 5.77492)))
    def alpha078(self):
        adv40 = sma(self.volume, 40)
        return (rank(
    ...
    # Alpha#79	 (rank(delta(IndNeutralize(((close * 0.60733) + (open * (1 - 0.60733))),IndClass.sector), 1.23438)) < rank(correlation(Ts_Rank(vwap.py, 3.60973), Ts_Rank(adv150,9.18637), 14.6644)))
    ...
    # Alpha#81	 ((rank(Log(product(rank((rank(correlation(vwap.py, sum(adv10, 49.6054),8.47743))^4)), 14.9655))) < rank(correlation(rank(vwap.py), rank(volume), 5.07914))) * -1)
    def alpha081(self):
    ...
    # Alpha#83	 ((rank(delay(((high - low) / (sum(close, 5) / 5)), 2)) * rank(rank(volume))) / (((high -low) / (sum(close, 5) / 5)) / (vwap.py - close)))
    def alpha083(self):
    ...
    # Alpha#84	 SignedPower(Ts_Rank((vwap.py - ts_max(vwap.py, 15.3217)), 20.7127), delta(close,4.96796))
    def alpha084(self):
    ...
    # Alpha#86	 ((Ts_Rank(correlation(close, sum(adv20, 14.7444), 6.00049), 20.4195) < rank(((open+ close) - (vwap.py + open)))) * -1)
    def alpha086(self):
        adv20 = sma(self.volume, 20)
        # 后者乘以20，使比较双方处于同一水平
    ...
    # Alpha#87	 (max(rank(decay_linear(delta(((close * 0.369701) + (vwap.py * (1 - 0.369701))),1.91233), 2.65461)), Ts_Rank(decay_linear(abs(correlation(IndNeutralize(adv81,IndClass.industry), close, 13.4132)), 4.89768), 14.4535)) * -1)
    ...
    # Alpha#89	 (Ts_Rank(decay_linear(correlation(((low * 0.967285) + (low * (1 - 0.967285))), adv10,6.94279), 5.51607), 3.79744) - Ts_Rank(decay_linear(delta(IndNeutralize(vwap.py,IndClass.industry), 3.48158), 10.1466), 15.3012))

    # Alpha#90	 ((rank((close - ts_max(close, 4.66719)))^Ts_Rank(correlation(IndNeutralize(adv40,IndClass.subindustry), low, 5.38375), 3.21856)) * -1)

    # Alpha#91	 ((Ts_Rank(decay_linear(decay_linear(correlation(IndNeutralize(close,IndClass.industry), volume, 9.74928), 16.398), 3.83219), 4.8667) -rank(decay_linear(correlation(vwap.py, adv30, 4.01303), 2.6809))) * -1)
    ...
    # Alpha#93	 (Ts_Rank(decay_linear(correlation(IndNeutralize(vwap.py, IndClass.industry), adv81,17.4193), 19.848), 7.54455) / rank(decay_linear(delta(((close * 0.524434) + (vwap.py * (1 -0.524434))), 2.77377), 16.2664)))

    # Alpha#94	 ((rank((vwap.py - ts_min(vwap.py, 11.5783)))^Ts_Rank(correlation(Ts_Rank(vwap.py,19.6462), Ts_Rank(adv60, 4.02992), 18.0926), 2.70756)) * -1)
    def alpha094(self):
        adv60 = sma(self.volume, 60)
    ...
    # Alpha#96	 (max(Ts_Rank(decay_linear(correlation(rank(vwap.py), rank(volume), 3.83878),4.16783), 8.38151), Ts_Rank(decay_linear(Ts_ArgMax(correlation(Ts_Rank(close, 7.45404),Ts_Rank(adv60, 4.13242), 3.65459), 12.6556), 14.0365), 13.4143)) * -1)
    def alpha096(self):
        adv60 = sma(self.volume, 60)
        ...
        return -1 * max(p1, p2)
        # return (max(ts_rank(decay_linear(correlation(rank(self.vwap.py), rank(self.volume).to_frame(), 4),4).CLOSE, 8), ts_rank(decay_linear(ts_argmax(correlation(ts_rank(self.close, 7),ts_rank(adv60, 4), 4), 13).to_frame(), 14).CLOSE, 13)) * -1)

    # Alpha#97	 ((rank(decay_linear(delta(IndNeutralize(((low * 0.721001) + (vwap.py * (1 - 0.721001))),IndClass.industry), 3.3705), 20.4523)) - Ts_Rank(decay_linear(Ts_Rank(correlation(Ts_Rank(low,7.87871), Ts_Rank(adv60, 17.255), 4.97547), 18.5925), 15.7152), 6.71659)) * -1)

    # Alpha#98	 (rank(decay_linear(correlation(vwap.py, sum(adv5, 26.4719), 4.58418), 7.18088)) -rank(decay_linear(Ts_Rank(Ts_ArgMin(correlation(rank(open), rank(adv15), 20.8187), 8.62571),6.95668), 8.07206)))
    def alpha098(self):
...
def start2():
    ...
    datas['vwap.py'] = datas.close / datas.volume / 100
