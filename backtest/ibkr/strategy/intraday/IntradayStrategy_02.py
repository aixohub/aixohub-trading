...
class IntradayStrategy(bt.Strategy):
    ...
    def __init__(self):
        ...
        self.rsi_ema = bt.indicators.EMA( self.rsi, period=10)

        # 1分钟线指标
        self.rsi_1m = bt.indicators.RSI(self.datas[0].close, period=self.p.rsi_period)
        self.kdj_1m = KDJ(self.datas[0])

        # 5分钟线指标（趋势过滤）
        self.rsi_5m = bt.indicators.RSI(self.datas[1].close, period=self.p.rsi_period)
        self.kdj_5m = KDJ(self.datas[1])
    ...
    def next(self):
        ...
        if not self.has_position:
            ...
            # 2. RSI从超卖区回升（<30）
            # 3. 价格在VWAP之上（多头趋势）
            if (self.rsi_1m < self.p.rsi_low and
                    self.kdj_1m.k[0] > self.kdj_1m.d[0] and self.kdj_1m.k[-1] <= self.kdj_1m.d[-1] and
                    self.rsi_5m > 50 and self.kdj_5m.k > self.kdj_5m.d):
                # 计算下单数量（使用95%的资金）
                size = self.broker.getcash() * self.params.position_size / self.data.close[0]
                self.position_size = int(size / 100) * 100  # 取整百股

                self.log(f'BUY CREATE, Size: {self.position_size}, Price: {self.data.close[0]:.2f}  rsi: {self.rsi[0]}   rsi_ema: {self.rsi_ema[0]}')
            ...
            # 卖出条件2：RSI超买
            sell_signal2 =(self.rsi[0] > 70  and self.rsi[0] - self.rsi_ema[0] < 5)

            # 卖出条件3：价格跌破VWAP
            sell_signal3 =  self.kdj.k[0] < self.kdj.d[0] and self.kdj.k[-1] >= self.kdj.d[-1]
